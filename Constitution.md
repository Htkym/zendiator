...
Closed request routes without behaviors can use a compact typed cache when they share one public reference-type handler and the mediator has no other route kinds. Other configurations retain the general cache. Generated general caches assign service-slot numbers per mediator composition, so unrelated compositions cannot make a mediator's pages sparse. Both cache forms preserve lazy capture and DI ownership.
...
- The generated `ZendiatorServiceResolver<TMediator>` belongs to a mediator instance. Give the same service type one slot within that composition and resolve it from the mediator's bound provider when first needed.
