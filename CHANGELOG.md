...
標準 DI と依存オブジェクトの遅延取得を使う実行経路に統一した。V1 前のプレビューであり、依存オブジェクトの再利用に破壊的変更がある。

- 各 Mediator は Handler・Behavior をサービス型ごとに最初に取得したインスタンスで保持する。Transient 登録も対象であり、同じ Mediator での再送やリトライは取得済みを使い回す。
- 生成と破棄は DI が担う。既定の登録は Scoped のまま。送信や列挙が終わるまでスコープを維持する。
- Roslyn の解析結果を内容で比較できる不変の出力モデルへ変換し、等しければテンプレート展開を省く。インターフェイス走査と型名の再利用、登録位置の分離を含む。
- 日英の README と MediatR 移行ガイドを更新し、設計・保守方針の Constitution を追加した。
- 263 件のテスト、24 プロジェクトの format、win-x64 Native AOT（AOT01〜AOT12）、package consumer 検証を通過した。値型で閉じたオープンジェネリック DI の制限は残る。
- 他ライブラリとの新しい正式な性能順位は公表しない。0.1.0 の性能記録は今回の設計の測定値ではない。
