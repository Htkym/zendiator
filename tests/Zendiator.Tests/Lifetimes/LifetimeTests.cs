...
public sealed class LifetimeTests
{
    ...
    [Fact]
    public async Task Transient_creates_new_dependencies_per_mediator()
    {
        var services = new ServiceCollection();
        ...
        var mediator = scope.ServiceProvider.GetRequiredService<IZendiator>();
        var identity = await mediator.SendAsync(new Identity());
        Assert.Equal(identity, await mediator.SendAsync(new Identity()));
        var nextMediator = scope.ServiceProvider.GetRequiredService<IZendiator>();
        Assert.NotSame(mediator, nextMediator);
        Assert.NotEqual(identity, await nextMediator.SendAsync(new Identity()));
    }
